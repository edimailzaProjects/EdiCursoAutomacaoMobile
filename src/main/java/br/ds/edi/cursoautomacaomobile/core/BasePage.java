...
public class BasePage {
	...
	public void tapa(int x, int y) {
		//new TouchAction(DriverFactory.getDriver()).tap(x, y).perform();
		new TouchAction<>(getDriver()).press(PointOption.point(x,y)).perform();
	}
	
	 public MobileElement scrollToElement(String elemento) {	  
			return (MobileElement) ((FindsByAndroidUIAutomator) DriverFactory.getDriver()).findElementByAndroidUIAutomator(
					"new UiScrollable(new UiSelector()" + ".scrollable(true)).scrollIntoView("
							+ "new UiSelector().description(\""+elemento+"\"));");
	  }
}
